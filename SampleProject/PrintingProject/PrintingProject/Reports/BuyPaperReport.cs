namespace PrintingProject.Reports
{
    ...
    /// </summary>
    public partial class BuyPaperReport : Telerik.Reporting.Report
    {
        BuyPaperFactory _buyPaperFactory = new BuyPaperFactory();
        public BuyPaperReport()
        {
            InitializeComponent();
        }
        
        private void BuyPaperReport_NeedDataSource(object sender, EventArgs e)
        {
            var report = sender as Telerik.Reporting.Processing.Report;
            int id = Convert.ToInt32(ReportParameters["Id"].Value);
            var dt = _buyPaperFactory.ReportById(id);
