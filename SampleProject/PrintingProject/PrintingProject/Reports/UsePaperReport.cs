namespace PrintingProject.Reports
{
    ...
    /// </summary>
    public partial class UsePaperReport : Telerik.Reporting.Report
    {
        UsePaperFactory _usePaperFactory = new UsePaperFactory();
        public UsePaperReport()
        {
            InitializeComponent();
        }
        

        private void UsePaperReport_NeedDataSource(object sender, EventArgs e)
        {
            var report = sender as Telerik.Reporting.Processing.Report;
            int id = Convert.ToInt32(ReportParameters["Id"].Value);
            var dt = _usePaperFactory.ReportById(id);
