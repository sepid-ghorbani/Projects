namespace PrintingProject.Reports
{
    ...
    /// </summary>
    public partial class FinalInvoiceReport : Telerik.Reporting.Report
    {
        FinalInvoiceFactory _finalInvoiceFactory = new FinalInvoiceFactory();
        public FinalInvoiceReport()
        {
            InitializeComponent();
        }

        private void FinalInvoiceReport_NeedDataSource(object sender, EventArgs e)
        {
            var report = sender as Telerik.Reporting.Processing.Report;
            int id = Convert.ToInt32(ReportParameters["Id"].Value);
            var dt = _finalInvoiceFactory.ReportById(id);
