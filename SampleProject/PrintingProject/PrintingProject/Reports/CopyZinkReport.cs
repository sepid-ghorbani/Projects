namespace PrintingProject.Reports
{
    ...
    /// </summary>
    public partial class CopyZinkReport : Telerik.Reporting.Report
    {
        CopyZinksFactory _copyZinksFactory = new CopyZinksFactory();
        public CopyZinkReport()
        {
            InitializeComponent();
        }

        private void CopyZinkReport_NeedDataSource(object sender, EventArgs e)
        {
            var report = sender as Telerik.Reporting.Processing.Report;
            int id = Convert.ToInt32(ReportParameters["Id"].Value);
            var dt = _copyZinksFactory.ReportById(id);
