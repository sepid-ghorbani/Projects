namespace PrintingProject.Reports
{
    ...
    /// </summary>
    public partial class LetterPressReport : Telerik.Reporting.Report
    {
        LetterPressesFactory _letterPressesFactory = new LetterPressesFactory();
        public LetterPressReport()
        {
            InitializeComponent();
        }


        private void LetterPressReport_NeedDataSource(object sender, EventArgs e)
        {
            var report = sender as Telerik.Reporting.Processing.Report;
            int id = Convert.ToInt32(ReportParameters["Id"].Value);
            var dt = _letterPressesFactory.ReportById(id);
