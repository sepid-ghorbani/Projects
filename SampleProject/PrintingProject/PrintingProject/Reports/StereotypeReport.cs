namespace PrintingProject.Reports
{
    ...
    /// </summary>
    public partial class StereotypeReport : Telerik.Reporting.Report
    {
        StereotypesFactory _stereotypesFactory = new StereotypesFactory();
        public StereotypeReport()
        {
            InitializeComponent();
        }

        private void StereotypeReport_NeedDataSource(object sender, EventArgs e)
        {
            var report = sender as Telerik.Reporting.Processing.Report;
            int id = Convert.ToInt32(ReportParameters["Id"].Value);
            var dt = _stereotypesFactory.ReportById(id);
